...
app.post("/create-video", auth, async (req, res) => {
  try {
    ...
    tasks[taskId] = { status: "processing", fileUrl: null };

    let providerResult =
      provider === "vidfly"
        ? await createVideoWithVidfly({
            prompt,
            avatar,
            voice,
            duration,
          })
        : await createVideoWithSora({
            prompt,
            duration,
            aspectRatio,
          });
...
app.listen(PORT, () => console.log("🚀 HC Video API running on port", PORT));
